...
interface TripFiltersProps {
  ...
  onChange: (filters: TripFilterState) => void;
  onReset: () => void;
}

export function TripFilters({ filters, onChange, onReset }: TripFiltersProps): React.JSX.Element {
  const handleChange = (key: keyof TripFilterState, value: string) => {
  ...
  return (
    <div className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4 mb-6 space-y-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-sm font-medium text-slate-300">Filter Trips</h3>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <div>
          <input
            ...
            className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
          />
        </div>
